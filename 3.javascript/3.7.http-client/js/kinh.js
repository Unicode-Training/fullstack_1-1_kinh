...
const addFormEventSubmit = () => {
    ...
    formEl.addEventListener('submit', async (e) => {
        ...
        let status;
        if (isEditMode) {
            status = await updateProduct(editId, productBody);
        } else {
            status = await createProduct(productBody);
        }
        ...
        if (!status) {
            ...
            if (isEditMode) {
                msgEl.innerText = 'Không thể cập nhật sản phẩm';
            } else {
                msgEl.innerText = 'Không thể thêm sản phẩm lúc này hoặc thiếu thông tin';
            }
        } else {
